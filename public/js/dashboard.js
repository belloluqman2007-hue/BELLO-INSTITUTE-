...
(function () {
  ...
  async function renderRoute(root) {
    ...
    finally { try { enhanceTables(root.querySelector("#dashContent")); } catch (e) { /* non-fatal */ } }
  }
    ...
    content.innerHTML = `
      ...
      <div class="dash-card">
        ...
        ${users.length ? `<div class="dash-table-wrap"><table class="dash-table">
          ...
          <button class="dash-btn dash-btn-primary" data-nav-route="teachers/add">Add a teacher</button>
        </div></div>`}
      </div>`;
    bindRouteButtons(content);
