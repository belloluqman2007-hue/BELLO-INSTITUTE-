...
/* --------------------------- fee structures ---------------------------- */
router.get("/items", STAFF, asyncHandler(async (req, res) => {
...
async function assignFee(tid, feeId, body, userId) {
  ...
  for (const studentId of studentIds) {
    ...
    await db.run(
      ...
      [tid, feeId, studentId, clampNum(body.amount_due ?? fee.amount_ngn, 0, 999999999, n(fee.amount_ngn)), validDate(body.due_date) || fee.due_date, "due", userId]
    ).catch(async (e) => { // MySQL branch for the same tenant-unique operation
      if (!/ON CONFLICT|syntax/i.test(e.message || "")) throw e;
      await db.run("INSERT INTO fee_assignments (madrasa_id,fee_item_id,student_id,amount_due,due_date,status,assigned_by,updated_at) VALUES (?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON DUPLICATE KEY UPDATE amount_due=VALUES(amount_due),due_date=VALUES(due_date),status=VALUES(status),updated_at=CURRENT_TIMESTAMP", [tid,feeId,studentId,clampNum(body.amount_due ?? fee.amount_ngn,0,999999999,n(fee.amount_ngn)),validDate(body.due_date)||fee.due_date,"due",userId]);
    });
